      ...
      <div className="price-add">
        <h5 id="product-price" style={{ width: '100%' }}>
          ...
          {props.product.discountprice ? (
            <React.Fragment>
              <CurrencyFormat value={props.product.discountprice} displayType={'text'} thousandSeparator={true} suffix={' VND'} renderText={value => <div style={{ color: 'red', float: 'left' }}>{value}</div>} />
              <CurrencyFormat value={props.product.price} displayType={'text'} thousandSeparator={true} suffix={' VND'} renderText={value => <div style={{ color: 'rgba(0,0,0,0.4)', textDecoration: 'line-through', marginLeft: '5px', float: 'left' }}>{value}</div>} />
            </React.Fragment>
          ) : (
