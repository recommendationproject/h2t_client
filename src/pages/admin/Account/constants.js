export const UPDATE_ADMIN = 'UPDATE_ADMIN';
export const UPDATE_ADMIN_SUCCESS = 'UPDATE_ADMIN_SUCCESS';
export const UPDATE_ADMIN_FAIL = 'UPDATE_ADMIN_FAIL';

export const ADMIN_SIGNIN = 'ADMIN_SIGNIN';
export const ADMIN_SIGNIN_SUCCESS = 'ADMIN_SIGNIN_SUCCESS';
export const ADMIN_SIGNIN_FAIL = 'ADMIN_SIGNIN_FAIL';

export const MLTS = 'MLTS';
export const SIGN_OUT = 'SIGN_OUT';

export const ADMIN_CHANGE_IMAGE = 'ADMIN_CHANGE_IMAGE';
export const ADMIN_CHANGE_IMAGE_SUCCESS = 'ADMIN_CHANGE_IMAGE_SUCCESS';
export const ADMIN_CHANGE_IMAGE_FAIL = 'ADMIN_CHANGE_IMAGE_FAIL';