...
const Sidebar = (props) => {
  ...
  useEffect(() => {
    ...
    if ('itemHistory' in localStorage) {
      setDataHistory(JSON.parse(localStorage.getItem('itemHistory')));
      // fetchDataHistory(arrItemHistory);
    }
  }, [props.location.pathname]);
  return (
    <React.Fragment>
      {(dataHistory && dataHistory.length && props.location.pathname!=='/cart' && props.location.pathname!=='/signin' && props.location.pathname!=='/signup' && props.location.pathname!=='/acc') > 0 ? (
